...
public class MyCalendarResourceIT {
     ...
     *
     * @see com.laidongs.sba.gateway.repository.search.MyCalendarSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createMyCalendar() throws Exception {
        ...
        // Validate the MyCalendar in Elasticsearch
        verify(mockMyCalendarSearchRepository, times(1)).save(testMyCalendar);
    }
    ...
    public void createMyCalendarWithExistingId() throws Exception {
        ...
        // Validate the MyCalendar in Elasticsearch
        verify(mockMyCalendarSearchRepository, times(0)).save(myCalendar);
    }
    ...
    public void updateMyCalendar() throws Exception {
        ...
        reset(mockMyCalendarSearchRepository);
    ...
    public void deleteMyCalendar() throws Exception {
        ...
        // Validate the MyCalendar in Elasticsearch
        verify(mockMyCalendarSearchRepository, times(1)).deleteById(myCalendar.getId());
    }

    @Test
    @Transactional
    public void searchMyCalendar() throws Exception {
        // Initialize the database
        myCalendarService.save(myCalendar);
        when(mockMyCalendarSearchRepository.search(queryStringQuery("id:" + myCalendar.getId())))
            .thenReturn(Collections.singletonList(myCalendar));
        // Search the myCalendar
        restMyCalendarMockMvc.perform(get("/api/_search/my-calendars?query=id:" + myCalendar.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(myCalendar.getId().intValue())))
            .andExpect(jsonPath("$.[*].calDate").value(hasItem(DEFAULT_CAL_DATE.toString())))
