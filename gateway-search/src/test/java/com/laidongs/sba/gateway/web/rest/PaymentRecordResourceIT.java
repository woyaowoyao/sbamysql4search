...
public class PaymentRecordResourceIT {
     ...
     *
     * @see com.laidongs.sba.gateway.repository.search.PaymentRecordSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createPaymentRecord() throws Exception {
        ...
        // Validate the PaymentRecord in Elasticsearch
        verify(mockPaymentRecordSearchRepository, times(1)).save(testPaymentRecord);
    }
    ...
    public void createPaymentRecordWithExistingId() throws Exception {
        ...
        // Validate the PaymentRecord in Elasticsearch
        verify(mockPaymentRecordSearchRepository, times(0)).save(paymentRecord);
    }
    ...
    public void updatePaymentRecord() throws Exception {
        ...
        reset(mockPaymentRecordSearchRepository);
    ...
    public void deletePaymentRecord() throws Exception {
        ...
        // Validate the PaymentRecord in Elasticsearch
        verify(mockPaymentRecordSearchRepository, times(1)).deleteById(paymentRecord.getId());
    }

    @Test
    @Transactional
    public void searchPaymentRecord() throws Exception {
        // Initialize the database
        paymentRecordService.save(paymentRecord);
        when(mockPaymentRecordSearchRepository.search(queryStringQuery("id:" + paymentRecord.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(paymentRecord), PageRequest.of(0, 1), 1));
        // Search the paymentRecord
        restPaymentRecordMockMvc.perform(get("/api/_search/payment-records?query=id:" + paymentRecord.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(paymentRecord.getId().intValue())))
            .andExpect(jsonPath("$.[*].txnType").value(hasItem(DEFAULT_TXN_TYPE.toString())))
