...
public class TrainingRecordResourceIT {
     ...
     *
     * @see com.laidongs.sba.gateway.repository.search.TrainingRecordSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createTrainingRecord() throws Exception {
        ...
        // Validate the TrainingRecord in Elasticsearch
        verify(mockTrainingRecordSearchRepository, times(1)).save(testTrainingRecord);
    }
    ...
    public void createTrainingRecordWithExistingId() throws Exception {
        ...
        // Validate the TrainingRecord in Elasticsearch
        verify(mockTrainingRecordSearchRepository, times(0)).save(trainingRecord);
    }
    ...
    public void updateTrainingRecord() throws Exception {
        ...
        reset(mockTrainingRecordSearchRepository);
    ...
    public void deleteTrainingRecord() throws Exception {
        ...
        // Validate the TrainingRecord in Elasticsearch
        verify(mockTrainingRecordSearchRepository, times(1)).deleteById(trainingRecord.getId());
    }

    @Test
    @Transactional
    public void searchTrainingRecord() throws Exception {
        // Initialize the database
        trainingRecordService.save(trainingRecord);
        when(mockTrainingRecordSearchRepository.search(queryStringQuery("id:" + trainingRecord.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(trainingRecord), PageRequest.of(0, 1), 1));
        // Search the trainingRecord
        restTrainingRecordMockMvc.perform(get("/api/_search/training-records?query=id:" + trainingRecord.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(trainingRecord.getId().intValue())))
            .andExpect(jsonPath("$.[*].status").value(hasItem(DEFAULT_STATUS.toString())))
