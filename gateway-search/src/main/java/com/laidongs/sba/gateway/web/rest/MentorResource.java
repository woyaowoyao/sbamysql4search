...
public class MentorResource {
    ...
    /**
     * {@code SEARCH  /_search/mentors?query=:query} : search for the mentor corresponding
     * to the query.
     *
     * @param query the query of the mentor search.
     * @return the result of the search.
     */
    @GetMapping("/_search/mentors")
    public List<Mentor> searchMentors(@RequestParam String query) {
        log.debug("REST request to search Mentors for query {}", query);
        return mentorService.search(query);
    }
}
