...
public class TrainingRecordResource {
    ...
    /**
     * {@code SEARCH  /_search/training-records?query=:query} : search for the trainingRecord corresponding
     * to the query.
     *
     * @param query the query of the trainingRecord search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/training-records")
    public ResponseEntity<List<TrainingRecord>> searchTrainingRecords(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of TrainingRecords for query {}", query);
        Page<TrainingRecord> page = trainingRecordService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
